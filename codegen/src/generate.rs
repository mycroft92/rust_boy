...
use crate::inst_parser::{Flags, Instruction, Time};
use log::{debug, error};
use serde_yaml;
...
fn dest_eval(val: &Value, map: &HashMap<String, Value>) -> tera::Result<Value> {
    ...
    let search = vec![
        "a", "b", "c", "d", "e", "f", "h", "l", "pc", "sp", "bc", "de", "hl",
    ];
    ...
    let out = match (&val).starts_with("(") {
        ...
        true => {
            if is_mem(&val[1..val.len() - 1], search) {
                format!(
                    "mmu.write{}(self.get_{}(),v); ",
                    bits,
                    &val[1..val.len() - 1]
                )
            } else if &val[1..val.len() - 1] == "a16" {
                format!(
                    "mmu.write{}(mem.read16(self.get_pc()), v); /*update pc by 2*/",
                    bits
                )
            } else {
                format!( "/*Default*/mmu.write{}(mem.read{}(self.get_pc() as u16) ,v);  /*update pc by 1*/", bits, bits)
            } //a8 case
        } // just take the first char,
        false => {
            if is_mem(&val[0..], search) {
                format!("self.set_{}(v);", val)
            } else {
                val
            }
        }
...
fn src_eval(val: &Value, map: &HashMap<String, Value>) -> tera::Result<Value> {
    ...
    let out = match (&val).starts_with("(") {
        ...
        true => {
            if is_mem(&val[1..val.len() - 1], search) {
                format!(
                    " let v: u8 = mmu.read8(self.get_{}());  ",
                    &val[1..val.len() - 1]
                )
            } else if &val[1..val.len() - 1] == "a8" {
                format!("let v: u16 = mmu.read8(self.get_pc()); /*To update pc by 1*/")
            } else if is_mem(&val[1..val.len() - 1], immediate) {
                format!( "/*Default true*/ let v = mmu.read16(self.get_pc()) /*{} To update pc by 2 */; ",val)
            } else {
                error!("Unexpected operand! : {}", val);
                val
            }
        } // just take the first char,
        false => {
            if is_mem(&val[0..], search) {
                format!("let v: u{} = self.get_{}();", bits, val)
            } else if is_mem(&val[0..], immediate) {
                format!("let v: u16 = mmu.read16(self.get_pc()); /*Update PC by 2*/")
            } else if &val[0..] == "d8" {
                format!("let v: u8 = mmu.read8(self.get_pc()); /*Update PC by 1*/")
            } else {
                error!("Unexpected operand! : {}", val);
                val
            }
...
fn set_flag(val: &Value, map: &HashMap<String, Value>) -> tera::Result<Value> {
    ...
    if val == '-' {
        ...
        Ok(to_value(format!("self.set_f{}(true);", flag)).unwrap())
    } else if val == '0' {
        Ok(to_value(format!("self.set_f{}(false);", flag)).unwrap())
    } else {
        Ok(to_value(format!("self.set_f{}({});", flag, flag)).unwrap())
    }
}
...
pub fn generate(inst: &str, out: &str) -> tera::Result<()> {
    ...
    let mut o = File::create(&inst_path)
        .expect(&format!("Cannot open \"inst.rs\" in {} for writing!", out));
    write!(o, "{}", &output);

    //formats the file appropriately
    Command::new("rustfmt")
        .args(["--force", &inst_path])
        .output()
